...
import { GuardAuditModal } from '@/components/guards/GuardAuditModal';
...
export default function AprovacaoPage() {
  ...
  const [auditGuardId, setAuditGuardId] = useState<string | null>(null);

  const [approvingRoleFor, setApprovingRoleFor] = useState<string | null>(null);
  const [selectedRole, setSelectedRole] = useState('Segurança');

  const ROLES = ['Segurança', 'Supervisor', 'Coordenador', 'Suporte', 'Fiscal', 'Diretor'];

  const loadAssignments = async () => {
    if (!user) return;
    try {
      const data = await api.listAssignments({ status: ['pending', 'pending_guard_approval'] });
      // Fetch associated job details
      const withJobs = await Promise.all(
        data.map(async (assignment: any) => {
          try {
            const job = await api.getJob(assignment.jobId);
            if (userData?.role === 'client' && job.contractorId !== user.uid) return null;
  ...
  useEffect(() => {
    loadAssignments();
    const interval = setInterval(loadAssignments, 8000);
    return () => clearInterval(interval);
  }, [user, userData]);

  const handleApprove = async (assignment: AssignmentWithJob, role: string) => {
    if (!assignment.id) return;
    setProcessing(assignment.id);
    try {
      await api.approveAssignment(assignment.id, role);
      loadAssignments();
    } catch (error) {
      console.error('Error approving:', error);
    } finally {
      setProcessing(null);
      setApprovingRoleFor(null);
      setSelectedRole('Segurança');
    }
  };
  ...
  const handleReject = async (assignment: AssignmentWithJob) => {
    ...
    setProcessing(assignment.id);
    try {
      await api.rejectAssignment(assignment.id);
      loadAssignments();
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
            ...
            {assignments.map((assignment) => (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <button 
                    onClick={() => setAuditGuardId(assignment.guardId)}
                    className="w-full mt-4 flex items-center justify-center gap-2 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl text-sm font-bold transition-colors"
                  >
                ...
                <div className="px-5 pb-5">
                  {approvingRoleFor === assignment.id ? (
                    <div className="flex flex-col gap-2">
                      <p className="text-xs font-bold text-slate-700 mb-1">Qual o papel na operação?</p>
                      <select
                        value={selectedRole}
                        onChange={(e) => setSelectedRole(e.target.value)}
                        className="w-full p-2.5 rounded-xl border border-slate-200 text-sm font-medium focus:ring-2 focus:ring-[#192c4d]"
                      >
                        {ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                      </select>
                      <div className="flex gap-2 mt-2">
                        <button
                          onClick={() => { setApprovingRoleFor(null); setSelectedRole('Segurança'); }}
                          className="flex-1 py-2 border border-slate-200 text-slate-600 rounded-xl font-bold hover:bg-slate-50 text-sm"
                        >
                          Cancelar
                        </button>
                        <button
                          onClick={() => handleApprove(assignment, selectedRole)}
                          disabled={processing === assignment.id}
                          className="flex-1 flex items-center justify-center gap-2 py-2 bg-green-500 text-white rounded-xl font-bold hover:bg-green-600 text-sm"
                        >
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() => { setApprovingRoleFor(assignment.id!); setSelectedRole('Segurança'); }}
                        disabled={processing === assignment.id}
      ...
      {/* Guard Audit Modal */}
      <GuardAuditModal
        guardId={auditGuardId!}
        isOpen={!!auditGuardId}
        onClose={() => setAuditGuardId(null)}
        onApprove={() => {
          const assignment = assignments.find(a => a.guardId === auditGuardId);
          if (assignment && assignment.id) {
            setApprovingRoleFor(assignment.id);
            setAuditGuardId(null);
          }
        }}
        onReject={() => {
          const assignment = assignments.find(a => a.guardId === auditGuardId);
