...
export default function ManualCheckinPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const found = await api.searchGuard(name.trim(), cpf.trim());
      if (!found) {
      ...
      const guardAssignments = await api.getGuardAssignmentsWithJobs(found.id!, mode === 'checkin' ? ['approved'] : ['checked_in']);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (mode === 'checkin') {
        res = await api.manualCheckin({
          guardId: guard.id,
          assignmentId: selectedAssignment.assignment.id!,
          jobId: selectedAssignment.job.id!,
        });
      } else {
        res = await api.manualCheckout({
          guardId: guard.id,
          assignmentId: selectedAssignment.assignment.id!,
          jobId: selectedAssignment.job.id!,
          checkinAt: selectedAssignment.assignment.checkinAt!,
        });
      }
