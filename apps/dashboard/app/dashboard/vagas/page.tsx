...
export default function VagasPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Open jobs (future dates)
      const openJobs = allJobs.filter(j => {
        if (j.status !== 'open' || !j.date) return false;
        const jobDate = new Date(j.date + 'T00:00:00');
        return jobDate >= today;
      });
      setJobs(openJobs);
      setMyRequests(allAssignments);

      // My assigned jobs
      const activeJobIds = allAssignments
        .filter((a: any) => ['pending', 'approved', 'checked_in', 'pending_guard_approval'].includes(a.status))
        .map((a: any) => a.jobId);
      setMyJobIds(activeJobIds);

      const assignedJobs = allJobs.filter(j => j.id && activeJobIds.includes(j.id));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {(() => {
          // Merge open jobs + my assigned jobs, deduplicate by id
          const allJobsMap = new Map<string, Job>();
          jobs.forEach(j => { if (j.id) allJobsMap.set(j.id, j); });
          myJobs.forEach(j => { if (j.id && !allJobsMap.has(j.id)) allJobsMap.set(j.id, j); });
                    ...
                    <div className="text-center py-2">
                      <p className="text-xs text-slate-500 flex items-center justify-center gap-1">
                        ...
                        {request?.status === 'pending' ? 'Aguardando aprovação do contratante' : 
                         ...
                         request?.status === 'checked_in' ? '🔐 Você está em serviço nesta escala.' :
