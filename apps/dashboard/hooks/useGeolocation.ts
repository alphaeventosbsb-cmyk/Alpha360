...
export function useGeolocation(options: UseGeolocationOptions): GeolocationState {
  ...
  // Sync to API (update user document with lat/lng)
  const syncToApi = useCallback(async (lat: number, lng: number) => {
    const now = Date.now();
    if (now - lastFirestoreUpdateRef.current < updateInterval) return;
    lastFirestoreUpdateRef.current = now;

    try {
      await api.updateUser(guardId, {
        lat,
        lng,
        lastLocationUpdate: new Date().toISOString(),
      } as any);
    } catch (error) {
      console.error('[Geolocation] Error syncing to API:', error);
    }
    ...
    if (inside && !arrivalAlertSentRef.current && jobId && companyId) {
      ...
      try {
        ...
        try {
          ...
          // Continue without contractor ID
        }
  ...
  }, [enabled, guardId, syncToApi, saveLocationHistory, checkGeofence]);
