...
export function QRScanner({ onScan, onClose, title = 'Escanear QR Code' }: QRScannerProps) {
  ...
  const startScanner = async () => {
    ...
    // Give React a tick to mount the container with its full size
    // otherwise mobile browsers calculate 0x0 video dimensions
    setTimeout(async () => {
      try {
        ...
        setError(
          err?.message?.includes('NotAllowedError')
            ...
            : 'Erro ao acessar a câmera. Verifique se está usando HTTPS e conceda permissão.'
        );
      }
    }, 100); // 100ms delay to guarantee DOM layout refresh
  };
