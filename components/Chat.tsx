...
import { sendMessage, subscribeToMessages, uploadFile } from '@/lib/firestore-service';
...
export function Chat({ jobId, jobTitle }: ChatProps) {
  ...
  useEffect(() => {
    if (!jobId) return;
    const unsub = subscribeToMessages(jobId, (msgs) => {
      setMessages(msgs);
    });
    return () => unsub();
  }, [jobId]);
    ...
    <div className="flex flex-col h-full bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div className="p-3 border-t border-slate-200 flex items-end gap-2">
        ...
        <textarea
          ...
          rows={1}
          className="flex-1 px-4 py-2.5 bg-slate-100 rounded-xl border-0 resize-none outline-none focus:ring-2 focus:ring-[#192c4d] text-sm placeholder:text-slate-400"
        />
        <button
          onClick={handleSend}
