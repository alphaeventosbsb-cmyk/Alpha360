...
import { updateAlert } from '@/lib/firestore-service';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // ---- Guards (filtered by companyId) ----
    const guardsConstraints: any[] = [where('role', '==', 'guard')];
    if (companyId) guardsConstraints.push(where('companyId', '==', companyId));
    const guardsQ = query(collection(db, 'users'), ...guardsConstraints);
    ...
    const jobsConstraints: any[] = [];
    if (companyId) jobsConstraints.push(where('companyId', '==', companyId));
    jobsConstraints.push(orderBy('createdAt', 'desc'));
    const jobsQ = query(collection(db, 'jobs'), ...jobsConstraints);
    ...
    // ---- Alerts (filtered by companyId) ----
    const alertsConstraints: any[] = [];
    if (companyId) alertsConstraints.push(where('companyId', '==', companyId));
    alertsConstraints.push(orderBy('createdAt', 'desc'), limit(20));
    const alertsQ = query(collection(db, 'alerts'), ...alertsConstraints);
    unsubs.push(onSnapshot(alertsQ, (snap) => {
      let alerts = snap.docs.map(d => ({ id: d.id, ...d.data() }));

      if (userData?.role === 'client') {
        alerts = alerts.filter((a: any) => a.contractorId === user.uid);
      } else if (userData?.role === 'guard') {
        alerts = alerts.filter((a: any) => a.guardId === user.uid);
      }

      setActiveAlerts(alerts.filter((a: any) => a.status === 'active' || a.status === 'dispatching').length);
      setRecentAlerts(alerts.slice(0, 6));
      setMapAlerts(alerts.filter((a: any) => a.lat && a.lng && a.status !== 'resolved'));

      // Track SOS alerts for alarm
      const activeSOS = alerts.filter((a: any) => a.type === 'sos' && a.status === 'active');
      setSOSAlerts(activeSOS);

      if (userData?.role === 'client') {
        setClientStats(prev => ({ ...prev, alerts: alerts.length }));
      }
      if (userData?.role === 'guard') {
        setGuardStats(prev => ({ ...prev, myAlerts: alerts.length }));
      }
    }));

    // ---- Pending Approvals (filtered by companyId) ----
    const approvalsConstraints: any[] = [where('status', '==', 'pending')];
    if (companyId) approvalsConstraints.push(where('companyId', '==', companyId));
    const approvalsQ = query(collection(db, 'job_assignments'), ...approvalsConstraints);
    unsubs.push(onSnapshot(approvalsQ, (snap) => {
      let pending = snap.docs;
      if (userData?.role === 'client') {
        pending = pending.filter(d => {
          // Would need to cross-reference with jobs, simplified here
          return true;
        });
      }
    ...
    if (userData?.role === 'guard') {
      ...
      unsubs.push(onSnapshot(earningsQ, (snap) => {
        const total = snap.docs.reduce((s, d) => s + (d.data().dailyRate || 0), 0);
        setGuardStats(prev => ({ ...prev, earnings: total }));
      }));
    }
  ...
  useEffect(() => {
    if (userData?.role === 'guard') return; // Guards don't get the alarm
    if (sosAlerts.length > 0 && soundEnabled) {
      if (sosAlerts.length > prevSOSCountRef.current) {
        startAlarm();
        if (navigator.vibrate) navigator.vibrate([1000, 500, 1000]);
        // Send push notification for new SOS
