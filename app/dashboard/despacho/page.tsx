...
import { updateAlert, deleteAlert } from '@/lib/firestore-service';
...
export default function DespachoPage() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(activeQuery, (snapshot) => {
      ...
      // Fetch guard phones for new alerts
      data.forEach(alert => {
        if (alert.guardId && !guardPhones[alert.guardId]) {
          getDoc(doc(db, 'users', alert.guardId)).then(snap => {
            if (snap.exists() && snap.data().phone) {
              setGuardPhones(prev => ({ ...prev, [alert.guardId!]: snap.data().phone }));
            }
          }).catch(() => {});
        }
      });
  ...
  // Listen to recently resolved alerts
  useEffect(() => {
    if (!user) return;
    ...
    const unsub = onSnapshot(resolvedQuery, (snapshot) => {
      setResolvedAlerts(snapshot.docs.map(d => ({ id: d.id, ...d.data() } as Alert)));
    });
    return () => unsub();
  }, [user]);
  ...
  const handleDispatch = async (alertId: string) => {
    setResolving(alertId);
    try {
      await updateAlert(alertId, { status: 'dispatching' });
    } catch (error) {
