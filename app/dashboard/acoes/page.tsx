...
export default function AcoesPage() {
  ...
  useEffect(() => {
    ...
    // Find active check-in assignment
    const q = query(
    ...
    const unsub = onSnapshot(q, async (snapshot) => {
      if (!snapshot.empty) {
        const assignment = { id: snapshot.docs[0].id, ...snapshot.docs[0].data() } as JobAssignment;
        setActiveAssignment(assignment);

        // Fetch the associated job for site location
        try {
          const jobDoc = await getDoc(doc(db, 'jobs', assignment.jobId));
          if (jobDoc.exists()) {
            setActiveJob({ id: jobDoc.id, ...jobDoc.data() } as Job);
          }
        } catch {
