...
import { processPixPayment, createPixPayment } from '@/lib/firestore-service';
...
export default function FaturamentoPage() {
  ...
  useEffect(() => {
    ...
    if (userData?.role === 'guard') {
      ...
      unsubs.push(onSnapshot(pq, (snap) => {
        setPayments(snap.docs.map(d => ({ id: d.id, ...d.data() } as PixPayment)));
      }));
    } else {
      // Admin/client: all assignments
      const q = query(
        collection(db, 'job_assignments'),
        where('companyId', '==', userData?.companyId || ''),
        where('status', 'in', ['checked_out', 'checked_in', 'approved'])
      );
      unsubs.push(onSnapshot(q, (snap) => {
        setAssignments(snap.docs.map(d => ({ id: d.id, ...d.data() })));
        setLoading(false);
      ...
      const pq = query(collection(db, 'pix_payments'), where('companyId', '==', userData?.companyId || ''), orderBy('createdAt', 'desc'));
      unsubs.push(onSnapshot(pq, (snap) => {
        setPayments(snap.docs.map(d => ({ id: d.id, ...d.data() } as PixPayment)));
      }));
    }
  ...
  const handleProcessPayment = async (assignment: any) => {
    ...
    try {
      ...
      await processPixPayment(paymentId);
    } catch (error) {
